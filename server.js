...
app.get('/', checkAuthenticated, (req,res)=>{
    ...
    {
        connection.query('Select * from todo_lists',(error,lists)=>{
            if(error)
            {
                console.log("Error in fetching todo lists: ",error)
            }
...
app.get('/add-task',(req,res)=>{
    ...
    connection.query('Select * from todo_lists where user_id =?',[userId],(error,rows)=>{
        ...
        else
        {
            ...
            console.log(listId)
            res.render('addnewtask.ejs',{listNames : rows})
        }
    })
})
...
app.post('/add-task',(req,res)=>{
    try{
        ...
        connection.query('Insert into tasks set ?',{todo_list_id: listId,tasks_title: taskName, Description: taskDescription, completed_by: formattedDate},(error,result)=>{
            if(error)
            {
